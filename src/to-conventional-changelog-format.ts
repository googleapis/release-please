...
type SummaryNode = parser.Type | parser.Scope | parser.BreakingChange | parser.Text;
...
export default function toConventionalChangelogFormat (ast: parser.Message): parser.ConventionalChangelogCommit {
  ...
  if (body) {
    visit(body, 'text', (node: parser.Text, _i: number, parent: parser.Node) => {
  ...
  visitWithAncestors(ast, ['breaking-change'], (node: parser.BreakingChange, ancestors: parser.Node[]) => {
    ...
    switch (parent.type) {
      ...
      case 'body':
        ...
        visit(parent, ['text', 'breaking-change'], (node: parser.Text | parser.BreakingChange) => {
          // TODO(@bcoe): once we have \n tokens in tree we can drop this:
  ...
  visit(ast, ['footer'], (node: parser.Footer) => {
    ...
    visit(node, ['type', 'separator', 'text'], (node: parser.Type | parser.Separator | parser.Text) => {
      switch (node.type) {
        case 'type':
          // refs, closes, etc:
