...
function isValidSignoffUser(signoffUser: string) {
...
export function signoffCommitMessage(
  commitMessage: string,
  signoffUser: string
): string {
  if (!isValidSignoffUser(signoffUser)) {
    throw new Error(
      `The format of '${signoffUser}' is not a valid email address with display name`
    );
  }
