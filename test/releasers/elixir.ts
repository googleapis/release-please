...
import {Elixir} from '../../src/releasers/elixir';
...
const fixturesPath = './test/releasers/fixtures/elixir';
...
describe('Elixir', () => {
  ...
  describe('run', () => {
    it('creates a release PR', async function () {
      ...
      // A mix.exs exists already:
      const mixContents = readFileSync(
        resolve(fixturesPath, 'mix.exs'),
        'utf8'
      );
      getFileContentsStub.onCall(1).resolves({
        sha: 'abc123',
        content: Buffer.from(mixContents, 'utf8').toString('base64'),
        parsedContent: mixContents,
      });
