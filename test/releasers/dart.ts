...
import {Dart} from '../../src/releasers/dart';
...
const fixturesPath = './test/releasers/fixtures/dart';
...
describe('Dart', () => {
  ...
  describe('run', () => {
    it('creates a release PR', async function () {
      ...
      // A pubspec.yaml exists already:
      const pubspecContents = readFileSync(
        resolve(fixturesPath, 'pubspec.yaml'),
        'utf8'
      );

      getFileContentsStub.withArgs('pubspec.yaml', 'main').resolves({
        sha: 'abc123',
